...
const Operator = {
  ...
  template: `
      <h1 class="operator_name">{{operator.name}}</h1>
        <div class="operator_cards_wrap">
          <div  v-for="t in tariff">
            <router-link :to="{name:'tariffId', params: {id: t.id}}" class="operator_link">
                <div class="operator_card">
                  <h2 class="tariff_name">{{t.name}}</h2>
                  <p class="tariff_cost">{{t.cost}}</p>
                </div>
            </router-link>
          </div>
      </div>`,
